...
public class GraphActivity extends Activity implements
	...
	// These two strings represent the name of indicators when using dual
	// indicators.
	private String IndicatorNamey;
	private String IndicatorNamex;

	// This is the axiscount, we use this when using dual indicators to tell
	// what axis information we are
	// listening for.
	private int axiscount = 0;
	// This count is used when deciding if the call that the datespinner
	// activates is null and to not be used
	private int accessseekbarcount = 0;
	// Used for the seekbar, to get the correct year and the number of years to
	// display.
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		countries = (ArrayList<Country>) getIntent().getSerializableExtra(
				"countries");
		...
		/*
		 * This spinner is kept invisible till unlocked when the dual indicators
		 * are unlocked.
		 */
		...
		// We add adapters to the x and y spinners, to edit the labels to match
		// the correct chosen label.
		setXAdapterDate();
		...
		// We create the adapter for the list view, using array indicatorName to
		// fill the list.
		ArrayAdapter<CharSequence> adapter2 = ArrayAdapter.createFromResource(
				this, R.array.indicatorName,
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		// We save the lockstatus, seekbarstatus, x and y indicators and if a
		// graph has been created.
		// We also give the scattergraph and linegraph to easily repaint the
		// view onto the activity.
	...
	protected void onRestoreInstanceState(Bundle savedState) {
		...
		// orientation
		/*
		 * We get the lock, if the seekbar is visible, if the graphs exists, the
		 * position of the indicators the year that is currently shown on the
		 * textview, the position of the seekbar if it exists, the graph itself
		 * as two graphical views.
		 */
		...
		scatterGraph = (ScatterGraph) savedState
				.getSerializable("scattergraph");
		...
		if (lineGraphExists == true) {
			...
			layout.addView(graph.getLineView(this), new LayoutParams(
		...
		if (scatterGraphExists == true) {
			// We add the view of the final graph.
			layout.removeAllViews();
			layout.addView(scatterGraph.getScatterGraph(this),
	 ...
	 * 
	 * @param message
	 *            The message that we want to tell the user.
	 */
	public void createMissingDialog(String message) {
		// We create a dialog to confirm the user wants to open the lock and use
		// dual indicators.

		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		LayoutInflater inflater = this.getLayoutInflater();
		builder.setView(inflater
				.inflate(R.layout.confirmindicator_dialog, null));
		...
		alertDialog.show();
		TextView description = (TextView) alertDialog
	...
	/**
	 * Class defines the time seekbar, changing the graph everytime the time
	 * changes.
	 * 
	 * @author Sean
	 */
		...
		public void onProgressChanged(SeekBar seekBar, int progress,
			...
			for (int i = 0; i < numberOfYears; i++) {
				...
				if (datesSeekBar.getProgress() == i) {
					...
					if (accessseekbarcount == 0) {// Do nothing, the graph has
													// not been created yet to
													// modify with the seekbar.
					} else {
						// We remove the old graph from view and set up the
						// renderer and dataset.
						layout.removeAllViews();
						...
						// We set the minimum and maximum values for the graphs.
						// This is to prevent them from changing each time the
						// slider changes.
						scatterGraph.setXAxisMinMax(xaxisminmax[0],
								xaxisminmax[1]);
						scatterGraph.setYAxisMinMax(yaxisminmax[0],
								yaxisminmax[1]);
						// We add the view of the final graph.
						layout.addView(scatterGraph
								.getScatterGraph(getApplicationContext()),
								new LayoutParams(LayoutParams.FILL_PARENT,
										LayoutParams.FILL_PARENT));
	...
	/**
	 * This method is used by the switch indicators imageview, to execute the
	 * animation and to switch the indicators in the spinners.
	 * 
	 * @param v
	 */
	 ...
	 * 
	 * @param scatterexists
	 *            Does a scattergraph already exist in viewport.
	 * @param year
	 *            The year that we want to set the date text at (Usually 1970).
	 * @param removeView
	 *            Do we want to remove all views out of graph(Usually yes,
	 *            unless orientation change).
	 */
	public void setDualIndicator(boolean scatterexists, String year,
			boolean removeView) {
		...
		// We set the adapter up, and change the lock image to an unlocked
		// image.
		setXAdapterArray();
		// We change the lock image to an unlocked image.
		lock.setImageResource(R.drawable.unlock);
		// We change the colour of the switch indicator, indicating it is now
		// possible to switch.
		switchindicator.setImageResource(R.drawable.switchindicatorblue);

		if (removeView == true) {
			// Remove all views out of the graph if there was any ready for the
			// next graph.
			layout.removeAllViews();
		}
	...
	public void lock(View v) {
		// This if statement checks if the lock is already open and if the
		// update method is running.
		if (isOpen == false && update.isEnabled() == true) {
			// We create a dialog to confirm the user wants to open the lock and
			// use dual indicators.

			AlertDialog.Builder builder = new AlertDialog.Builder(this);
			LayoutInflater inflater = this.getLayoutInflater();
			builder.setView(inflater.inflate(R.layout.confirmindicator_dialog,
					null));
			...
			// We set the spinner back to 0 for the next instance of using dual
			// indicators.
			accessseekbarcount = 0;
			// We set the x indicator back to the date array, to remove the rest
			// of the indicators.
			setXAdapterDate();
			// We set the lock back to close.
			lock.setImageResource(R.drawable.lock);
			// We change the colour of the switch indicator, indicating it is
	...
	/**
	 * This method adds adapters to the X and Y spinners based on the date array
	 * in strings.xml.
	 */
	public void setXAdapterDate() {
		...
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(
				this, R.array.date, android.R.layout.simple_spinner_item);
	...
	/**
	 * This method adds adapters to the X and Y spinners based on the
	 * indicatorName array in strings.xml.
	 */
	...
	/**
	 * Method handles updating the graph with new indicators.Handles all UI
	 * changes.
	 */
	public void updateGraph() {
		if (isInternetAvailable()) {
			// We prevent the user from rotating the screen, causing issues with
			// the parser sending information after the activity is destroyed.
			...
			if (isOpen == true) {
				...
				IndicatorNamey = getResources().getStringArray(
						R.array.indicatorID)[IndicatorPosy];
				IndicatorNamex = getResources().getStringArray(
						R.array.indicatorID)[IndicatorPosx];
				...
				for (Country country : countries) {
					parser.getIndicatorFor(country.getId(), IndicatorNamey,
							"1970", "2012");
					parser.getIndicatorFor(country.getId(), IndicatorNamex,
							"1970", "2012");
				}
				...
				IndicatorName = getResources().getStringArray(
						R.array.indicatorID)[IndicatorPos];
				// We create the parser object
				JSONParser parser = new JSONParser(this);
				// We reset the countriescount
				countriescount = 0;
				// We iterate through the countries and get the indicators map.
				for (Country country : countries) {
					parser.getIndicatorFor(country.getId(), IndicatorName,
							"1970", "2012");
				}
			}
	...
	/**
	 * This method updates the graph based on the values in the LineGraph
	 * object.
	 * 
	 * @param v
	 */
	...
	/**
	 * This class is a listener. On the data being parsed and created into an
	 * object, this class is called. We add data to the graph once it is called.
	 * Once all data is added, it creates the graph view and adds it to the
	 * layout.
	 * 
	 * @param type
	 * @param result
	 *            The resulting map of data.
	 */
	@SuppressWarnings("deprecation")
	@Override
	public void onJSONParseFinished(String type, HashMap result) {
		// We check if the lock is opened, and the information is dual
		// indicators or just a single indicator (isOpen is false).
		if (isOpen == true) {
			// This means that the data that has finished parsing is for the X
			// Axis.
			if (axiscount == 0) {
				if (((Indicator) result.get(IndicatorNamey)).getValues().size() > 0) {
					...
					scatterGraph.addYDataSet(hashmap,
							countries.get(countriescount));
					// Increment the value to show the next dataset is for the y
					// axis.
				} else {
				...
				// This means the data that has finished parsing is for the Y
				// Axis.
				if (((Indicator) result.get(IndicatorNamex)).getValues().size() > 0) {
					...
					scatterGraph.addXDataSet(hashmap,
							countries.get(countriescount));
					// We add the data to the graph. We using countriescount to
					// keep track of what countries data we are adding.
					scatterGraph.addDataToGraph(countries.get(countriescount),
							year);

					// We compare the count to the size of the arraylist, to see
					// if we have finished adding the data.
					if (countriescount == (countries.size() - 1)) {
						...
						/*
						 * We get the layout for the graph, remove all views
						 * from the graph. We add the new graph to the layout in
						 * the form of a view. We also set the loading animation
						 * to be transparent.
						 */
						dialog.dismiss();
						layout.removeAllViews();
						...
						// The spinner can now be used, since an update has
						// occured.
						accessseekbarcount = 1;
					...
					// Make sure we update the countries count even we don't
					// have internet
					if (countriescount != (countries.size() - 1)) {
						countriescount++;
					}
				}
				// We have finished parsing this countries data, so we reset the
				// count.
				axiscount = 0;
			}
		} else {
			// This means that the data we are getting is for a single
			// indicator.
			...
			// We compare the count to the size of the arraylist, to see if we
			// have finished adding the data.
			if (countriescount == (countries.size() - 1)) {
				// We add the correct amount of renderers.
				graph.addRenderers();

				/*
				 * We get the layout for the graph, remove all views from the
				 * graph. We add the new graph to the layout in the form of a
				 * view. We also set the loading animation to be transparent.
				 */
				dialog.dismiss();
				layout.removeAllViews();
				layout.addView(graph.getLineView(getApplicationContext()),
						new LayoutParams(LayoutParams.FILL_PARENT,
								LayoutParams.FILL_PARENT));
				// The update button can now be pressed again.
				update.setEnabled(true);
	...
	/**
	 * Alex's method to check if internet is available. We check before pressing
	 * update to make sure there is an internet connection.
	 * 
